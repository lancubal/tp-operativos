...
static void* serializar_aprobar_operativos(uint8_t nota1, uint8_t nota2) {
    void* stream = malloc(sizeof(op_code_NUESTRO) + sizeof(uint8_t) * 2);

    op_code_NUESTRO cop = APROBAR_OPERATIVOS;
    memcpy(stream, &cop, sizeof(op_code_NUESTRO));
    memcpy(stream+sizeof(op_code_NUESTRO), &nota1, sizeof(uint8_t));
    memcpy(stream+sizeof(op_code_NUESTRO)+sizeof(uint8_t), &nota2, sizeof(uint8_t));
    return stream;
}
...
bool send_aprobar_operativos(int fd, uint8_t nota1, uint8_t nota2) {
    size_t size = sizeof(op_code_NUESTRO) + sizeof(uint8_t) * 2;
...
static void* serializar_mirar_netflix(size_t* size, char* peli, uint8_t cant_pochoclos) {
    ...
    *size =
    ...
    op_code_NUESTRO cop = MIRAR_NETFLIX;
    memcpy(stream, &cop, sizeof(op_code_NUESTRO));
    memcpy(stream+sizeof(op_code_NUESTRO), &size_payload, sizeof(size_t));
    memcpy(stream+sizeof(op_code_NUESTRO)+sizeof(size_t), &size_peli, sizeof(size_t));
    memcpy(stream+sizeof(op_code_NUESTRO)+sizeof(size_t)*2, peli, size_peli);
    memcpy(stream+sizeof(op_code_NUESTRO)+sizeof(size_t)*2+size_peli, &cant_pochoclos, sizeof(uint8_t));
